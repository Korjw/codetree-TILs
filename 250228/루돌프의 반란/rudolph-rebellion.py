...
def rudol_crash(x,y,direction, t,stun_yn):
    if grid[x][y][0]:
        ...
        if grid[move_x][move_y][0]:
            santa_crash(move_x, move_y, direction,1,0)
    return

def santa_crash(x,y,direction,t,stun_yn):
    if grid[x][y][0]:
        santa_idx = santa.index([grid[x][y][0], x, y])
        if direction == -1:
...
for _ in range(3):
    ...
    rudol_crash(rudol[0], rudol[1],rudol[2],C,1)
    make_grid()

    santa_move()

    santa_crash(rudol[0], rudol[1],-1,D,1)
    make_grid()
    # 점수 기록
