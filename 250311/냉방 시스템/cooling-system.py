...
def wind(i,x,y,num,direction):
    if direction == 0: #좌
        ...
        if 2 not in wall_grid[x][y]: # 직진 가능
            dir_wx.append(0)
            dir_wy.append(-1)

        if not out_of_range(x+1,y) and 1 not in wall_grid[x+1][y] and 2 not in wall_grid[x+1][y]: # 아래로 가능
            dir_wx.append(1)
            dir_wy.append(-1)

        if 1 not in wall_grid[x][y] and not out_of_range(x-1,y) and 2 not in wall_grid[x-1][y]: # 위로 가능
            dir_wx.append(-1)
            dir_wy.append(-1)
...
#1 위쪽 벽 2 왼쪽벽
        ...
        if not out_of_range(x,y+1) and 2 not in wall_grid[x][y+1]: # 직진 가능
            dir_wx.append(0)
            dir_wy.append(1)

        if not out_of_range(x+1,y) and 1 not in wall_grid[x+1][y] and not out_of_range(x+1,y+1) and 2 not in wall_grid[x+1][y+1]: # 아래로 가능
            dir_wx.append(1)
            dir_wy.append(1)

        if 1 not in wall_grid[x][y] and not out_of_range(x-1,y+1) and 2 not in wall_grid[x-1][y+1]: # 위로 가능
            dir_wx.append(-1)
            dir_wy.append(1)
...
#1 위쪽 벽 2 왼쪽벽
        if 1 not in wall_grid[x][y]: # 직진가능
            dir_wx.append(-1)
            dir_wy.append(0)

        if 2 not in wall_grid[x][y] and not out_of_range(x,y-1) and 1 not in wall_grid[x][y-1]: # 왼쪽 가능
            dir_wx.append(-1)
            dir_wy.append(-1)

        if not out_of_range(x,y+1) and 1 not in wall_grid[x][y+1] and 2 not in wall_grid[x][y+1]: # 오른쪽 가능
            dir_wx.append(-1)
            dir_wy.append(1)
...
#1 위쪽 벽 2 왼쪽벽
        if not out_of_range(x+1,y) and 1 not in wall_grid[x+1][y]: # 직진가능
            dir_wx.append(1)
            dir_wy.append(0)

        if 2 not in wall_grid[x][y] and not out_of_range(x+1,y-1) and 1 not in wall_grid[x+1][y-1]: # 왼쪽 가능
            dir_wx.append(1)
            dir_wy.append(-1)

        if not out_of_range(x,y+1) and 2 not in wall_grid[x][y+1] and not out_of_range(x+1,y+1) and 1 not in wall_grid[x+1][y+1]: # 오른쪽 가능
            dir_wx.append(1)
            dir_wy.append(1)
            ...
            for k in range(n):
                ...
                for k in range(curr_lx,curr_hx+1):
                    for j in range(curr_ly, curr_hy+1):
                        ...
                        if air_con_grid[i][k][j]:
                            wind(i,k,j,count,air[2])
                curr_lx, curr_ly, curr_hx, curr_hy = curr_lx+dir_lx[3], curr_ly+dir_ly[3], curr_hx+dir_hx[3], curr_hy+dir_hy[3]
                count -= 1
            #break
...
#1 위쪽 벽 2 왼쪽벽
    for i in range(n):
        for j in range(n):
            ...
            if wall_grid[i][j] == [2]:
                dir_x, dir_y = [-1], [0]
            if wall_grid[i][j] == [1,2] or wall_grid[i][j] == [2,1]:
                continue
...
# for i in range(n):
#     for j in range(n):
#         print(air_con_grid[11][i][j], end = ' ')
#     print()
# print()

# for i in range(n):
#     for j in range(n):
#         print(sum(wall_grid[i][j]), end = ' ')
#     print()
# print()
